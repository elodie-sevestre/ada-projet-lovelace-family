...
- **`LoginForm`** — formulaire email / mot de passe, validation côté client (format email, mot de passe ≥ 8 caractères) avant l'appel `POST /auth/connexion`. Stocke le token reçu dans le `localStorage`, remonte l'info à `App.jsx` via `setToken`. État `loading` pendant la requête ; erreurs (401, erreur serveur) affichées sous le formulaire.
- **`LogoutButton`** — appelle `onLogout` (prop reçue de `App.jsx`). Ne gère ni la suppression du token ni le reset du state, portés par `App.jsx`.
- **`api/client.js`** — ajoute `Authorization: Bearer <token>` sur chaque requête (token relu depuis le `localStorage` à chaque appel). Réponse `401` → suppression du token et redirection vers `/`.
...
- **`TaskCheckBox`** — bascule le statut d'une tâche (`A_FAIRE` ↔ `TERMINE`) via checkbox stylée, déclenche une animation locale, appelle `onCelebrate` uniquement en passant à `TERMINE`. Flux API : [Flux applicatifs](../explications/flux-applicatifs.md#modification-dune-tâche).
...
- **Contrôle d'accès** : dans `TaskItem`, les boutons d'édition et de suppression ne sont rendus que si `currentUser.role === "ADMIN"`.

## Communication avec l'API

Tous les appels HTTP passent par `api/client.js`, qui centralise les headers JSON, le token et la gestion des erreurs (`response.ok`, `401`). Les modules comme `api/tasks.js` appellent `get` / `post` / `put` / `del` avec la route voulue.
